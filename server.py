#!/usr/bin/env python3
"""
Frontend POC - Visualización de datos históricos OHLCV.

Servidor simple que sirve una UI para explorar los tickers descargados.
Muestra listado con data points, fechas, y gráfico de cierre al hacer click.
Incluye reporte ejecutivo por tipo y estado de vencimiento de bonos.

Uso:
    python server.py                  # http://localhost:8080
    python server.py --port 3000      # http://localhost:3000
    python server.py --db-path data/historial.db
"""
...
def parse_bond_expiration(ticker: str, description: str) -> str | None:
    """Extract expiration date from bond description. Returns ISO date string or None."""
    if not description:
        return None

    # Pattern 1: 'vencimiento el DD de MES de YYYY' or 'vencimiento DD de MES del YYYY'
    m = re.search(
    ...
    # Pattern 2: 'venciendo el DD/MM' (short-term letters, year from ticker last digit)
...
<title>Scrapper Historico - POC</title>
<style>
...
async function loadChart(ticker) {
  ...
  const resp = await fetch(`/api/ohlcv/${ticker}`);
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _api_report(self):
        ...
        # Get all tickers with bar counts and data source
        rows = conn.execute("""
            SELECT
                t.ticker,
                t.trading_type,
    ...
    def _api_ohlcv(self, ticker):
        ...
        if not rows:
            self._send_json({"dates": [], "open": [], "high": [], "low": [], "close": [], "volume": []})
            return
...
def main():
    parser = argparse.ArgumentParser(description="Frontend POC - Scrapper Historico")
    parser.add_argument("--port", type=int, default=8080, help="Puerto (default: 8080)")
    parser.add_argument("--db-path", default=DEFAULT_DB, help="Ruta a la base de datos SQLite")
    args = parser.parse_args()

    Handler.db_path = args.db_path

    server = HTTPServer(("0.0.0.0", args.port), Handler)
    print(f"Servidor corriendo en http://localhost:{args.port}")
